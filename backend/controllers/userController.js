...
const { sendVarifiedMail, sendCode } = require("../helpers/nodemailer");
...
exports.userRegistrationController = async (req, res) => {
  try {
    ...
    if (alreadyUser) {
      ...
      return res
        .status(400)
        .json({ message: "please fname must have atlest 3 and max 15 word" });
    } else if (lengthValidation(lname, 3, 15)) {
      return res
        .status(400)
        .json({ message: "please lname must have atlest 3 and max 15 word" });
    } else if (!emailValidation(email)) {
      return res.status(400).json({ message: "invalid email" });
    } else if (lengthValidation(password, 8)) {
      return res.status(400).json({ message: "password must have atlest 8" });
    }
    ...
    bcrypt.hash(password, 10, function (err, hash) {
      ...
      sendVarifiedMail(user.email, user.fname, url);
      ...
      res.send({
        ...
        followers: followers,
        coverPicture: coverPicture,
        message: "registration successfull. please varify your email",
      });
    });
  } catch (error) {
...
exports.verifiedMail = async (req, res) => {
  try {
    ...
    if (verified !== user.id) {
      ...
      return res.status(200).json({ message: "successfully verified" }); // Change status to 200
    }
  } catch (error) {
    return res.status(400).json({ message: error.message });
  }
...
exports.reVerfiedEmail = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      message: "Email verification Link has been send your Email Account",
    });
  } catch (error) {
...
exports.resetCode = async (req, res) => {
  try {
    ...
    sendCode(user.email, user.fname, code);
    res.status(200).json({
      message: "Password Reset Code has been sent your email",
    });
  } catch (error) {
    return res.status(404).json({ message: error.message });
  }
